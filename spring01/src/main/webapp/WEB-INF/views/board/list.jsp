...
<h2>게시판</h2>
	<form name="form1" method="post" action="${path}/board/list.do">
		...
		</select> <input name="keyowrd" value="${map.keyword}"> <input
			...
			<c:choose>
				<c:when test="${row.show=='Y'}">
					<tr>
						<td>${row.bno}</td>
						<td><a
							href="${path}/board/view.do?bno=${row.bno}
			&curPage=${map.pager.curPage}
			&search_option=${map.search_option}
			&keyword=${map.keyword}">
								${row.title}</a> <c:if test="${row.cnt > 0}">
			...
			<td colspan="5" align="center"><c:if
					...
					<c:choose>
						<c:when test="${num == map.pager.curPage}">
							<!-- 현재 페이지이면 하이퍼링크 제거 -->
							<span style="color: red">${num}</span>&nbsp; </c:when>
						<c:otherwise>
							<a href="javascript:list('${num}')"> ${num}</a>&nbsp; </c:otherwise>
					</c:choose>
